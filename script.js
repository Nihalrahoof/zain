...
foodToggleButtons.forEach(button => {
  button.addEventListener('click', function () {
    ...
    if (activeFoodContent && activeFoodContent !== target) {
      activeFoodContent.style.display = 'none';
      activeFoodButton.classList.remove('active');
    }
    ...
    activeFoodContent = !isVisible ? target : null;
    activeFoodButton = !isVisible ? this : null;
  });
});

document.addEventListener('click', function (e) {
  const isFoodAccordionArea = e.target.closest('.foods-accordion-list');
  if (!isFoodAccordionArea && activeFoodContent) {
    activeFoodContent.style.display = 'none';
    activeFoodButton.classList.remove('active');
    activeFoodContent = null;
    activeFoodButton = null;
  }
});

// ✅ Timeline Animation (highlight steps on load)
window.addEventListener('DOMContentLoaded', () => {
  const steps = document.querySelectorAll('.timeline-step');
  steps.forEach((step, i) => {
    setTimeout(() => {
      step.classList.add('highlight');
      setTimeout(() => step.classList.remove('highlight'), 800);
    }, 400 * i);
  });
});
